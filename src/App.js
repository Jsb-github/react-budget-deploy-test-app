...
const App = () => {
  ...
  const handleDelete = (id) => {
    const newExpenses = expenses.filter((expence) => expence.id !== id);
    console.log(newExpenses);
    setExpenses(newExpenses);
  };

  const handleSubmit = (e) => {
    e.preventDefault(); // 페이지 리플렉시 방지

    if (charge !== "" && amount > 0) {
      //불변성을 지키기 위해 expense state을 업데이트하기 전에
      // 새로운 Expense를 만듭니다.
      if (edit) {
      ...
      handleAlert({
        type: "danger",
        text: `charge는 빈 값일 수 없으며 amunt 값은 0보다 커야 합니다`,
      });
    }
  };
